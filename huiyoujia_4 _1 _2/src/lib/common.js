...
function ajax(opt) {
    ...
    if (obj.type.toLowerCase() == 'get') {
        //get方式
        if (obj.data) {
            obj.data = objToStr(obj.data);
            obj.url += '?' + obj.data;
        }
        xhr.open('get', obj.url, obj.asyn);
        xhr.send(null);
    } else if (obj.type.toLowerCase() == 'post') {
        //post方式
        xhr.open('post', obj.url, obj.asyn);
...
//封装cookie
// 设置  document.cookie = name=value[;expires=date][;path=路径][;domain=域名]
// function setCookie(key, val, iDay) {
//     // //key : 键名 ； val : 键值 ； iDay ：失效时间
//     // var str = key + '=' + val + ';path=/';
//     // if (iDay) {
//     //     var date = new Date();
//     //     date.setDate(date.getDate() + iDay); //设置失效时间
//     //     str += ';exprise=' + date; //如果有这个参数就设置失效时间
//     // }
//     // document.cookie = str;
//       //key：键名；val：键值；iDay：失效时间
//       var now = new Date();
...
//     //传键名，返回对应的键值 例如 ：name = 小鱿鱼; age = 20
...
// //删除cookie -- 所谓删除，不是真正的删除，只要把它的时间设置为过去的就行，现在是正的，只要设置为负数即可
